...
# Master script to run the DPA completeness evaluation pipeline with Direct LLM approach
# This approach uses satisfaction-based classification where the LLM identifies which 
# requirements (if any) each segment SATISFIES, then processes through Deolingo solver.

set -e  # Exit on any error

# Configuration
DPA_CSV="data/test_set.csv"
REQUIREMENTS_FILE="data/requirements/requirements_deontic_manual.json"
OLLAMA_MODEL="llama3:70b"  # Default Ollama model
OUTPUT_DIR="results/direct_approach/gpt-oss"
DEOLINGO_RESULTS="${OUTPUT_DIR}/deolingo_results.txt"
EVALUATION_OUTPUT="${OUTPUT_DIR}/evaluation_results.json"
TARGET_DPAS=("Online 124" "Online 132" "Online 54" "Online 57" "Online 39" "Online 140" "Online 87")  # Array of DPAs to process
...
check_ollama() {
    ...
    if ! curl -f -s "${ollama_url}/api/tags" > /dev/null; then
        log "ERROR: Ollama server is not running at ${ollama_url}!"
        log "Please start Ollama server first"
        exit 1
    fi
    log "Ollama server is running at ${ollama_url}"
}
...
ensure_model() {
    ...
    # If OLLAMA_HOST is set to a remote server, assume model management is handled externally
    if [[ "${OLLAMA_HOST:-}" =~ ^http://ollama: ]]; then
        log "Using remote Ollama server, assuming model ${model} is available"
        return 0
    fi
...
set_requirements_file() {
    case $REQUIREMENTS_REPRESENTATION in
        ...
        "deontic_ai")
            ...
            ;;
        "deontic_short")
            REQUIREMENTS_FILE="data/requirements/requirements_deontic_short.json"
            ;;
        "deontic_verbose")
            REQUIREMENTS_FILE="data/requirements/requirements_deontic_verbose.json"
            ;;
        *)
            log "ERROR: Unknown requirements representation: $REQUIREMENTS_REPRESENTATION"
            log "Available options: deontic, deontic_ai, deontic_short, deontic_verbose"
            exit 1
            ;;
    esac
...
check_deolingo() {
    if ! command -v deolingo &> /dev/null; then
        ...
        log "Deolingo is required to run the solver (steps 2-3)."
        log ""
...
DEOLINGO_RESULTS="${OUTPUT_DIR}/deolingo_results_${MODEL_SAFE_NAME}_direct.txt"
EVALUATION_OUTPUT="${OUTPUT_DIR}/evaluation_results_${MODEL_SAFE_NAME}_direct.json"

echo "========== DPA Completeness Checker - Direct LLM Approach =========="
...
echo "1. Generate Direct LP files using satisfaction-based classification for all DPAs"
...
case ${STEP} in
    1)
        echo -e "\n[Step 1] Generating Direct LP files for ${MAX_SEGMENTS} segments..."
        ...
        for TARGET_DPA in "${TARGET_DPAS[@]}"; do
            ...
            python3 generate_direct_lp_files.py \
        ...
        echo "Step 1 completed. Direct LP files generated in: ${OUTPUT_DIR}/lp_files_[DPA_NAME]"
        ;;
    2)
        ...
        for TARGET_DPA in "${TARGET_DPAS[@]}"; do
            ...
            # Process all .lp files for this DPA (using req_* subdirectories)
    ...
    A|a)
        ...
        echo "Step 1 completed. Direct LP files generated."
